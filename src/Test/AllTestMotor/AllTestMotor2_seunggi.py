# AllTestMotor.py 코드에서 에러가 발생했을 때 디버깅을 위한 발생 위치, 발생 함수 등을 에러 문구에 추가하여 디버깅이 편리해졌고, 1번 모터를 제어할 때, '.'을 누르면 왼쪽으로 '/'을 누르면 오른쪽으로 회전하도록 수정.
...
def set_profile_velocity_acceleration(velocity, acceleration, ID):
    ...
    if dxl_comm_result != COMM_SUCCESS:
        print(f"[속도 및 가속도 세팅 함수] Error setting velocity to {ID}: {packetHandler.getTxRxResult(dxl_comm_result)}")
    elif dxl_error != 0:
        print(f"[속도 및 가속도 세팅 함수] Error: {packetHandler.getRxPacketError(dxl_error)}")
    
    # Set Profile Acceleration
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, ID, ADDR_PROFILE_ACCELERATION, acceleration)
    if dxl_comm_result != COMM_SUCCESS:
        print(f"[속도 및 가속도 세팅 함수] Error setting acceleration to {ID}: {packetHandler.getTxRxResult(dxl_comm_result)}")
    elif dxl_error != 0:
        print(f"[속도 및 가속도 세팅 함수] Error: {packetHandler.getRxPacketError(dxl_error)}")

#초기 위치 함수
# 연결 안 된 모터의 번호는 -1로 통일
def set_init_position(ID=-1, minPosition=0):
    # ID가 -1인 경우 함수를 종료하여 write4ByteTxRx 호출을 방지
    if ID == -1:
        print("모터 ID -1: 연결되지 않은 모터로 초기화 작업을 건너뜁니다.")
        return
    else:
        # ID가 유효한 경우 모터 위치 설정 시도
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, ID, ADDR_GOAL_POSITION, minPosition)
        
        # 결과 확인 및 출력
        if dxl_comm_result != COMM_SUCCESS:
            print("[초기 위치 설정 함수] %d번 모터, %s" % (ID, packetHandler.getTxRxResult(dxl_comm_result)))
        elif dxl_error != 0:
            print("[초기 위치 설정 함수] %d번 모터, %s" % (ID, packetHandler.getRxPacketError(dxl_error)))


# 토크 설정 함수
# 연결 안 된 모터의 번호는 -1로 통일
def Enable_Torque(ID=-1, T_ENABLE=1):
    # ID가 -1인 경우 함수를 종료하여 write1ByteTxRx 호출을 방지
    if ID == -1:
        print("모터 ID -1: 연결되지 않은 모터로 토크 설정을 건너뜁니다.")
        return
    else:
        # ID가 유효한 경우 토크 설정 시도
        ...
        # 결과 확인 및 출력
        if dxl_comm_result != COMM_SUCCESS:
            print("[토크 설정 함수] %d번 모터, %s" % (ID, packetHandler.getTxRxResult(dxl_comm_result)))
        elif dxl_error != 0:
            print("[토크 설정 함수] %d번 모터, %s" % (ID, packetHandler.getRxPacketError(dxl_error)))
        else:
...
# 토크 해제 함수
# 연결 안 된 모터의 번호는 -1로 통일
def Disable_Torque( ID=-1, T_ENABLE=0):
    # ID가 -1인 경우 함수를 종료하여 write1ByteTxRx 호출을 방지
    if ID == -1:
        print("모터 ID -1: 연결되지 않은 모터로 토크 해제를 건너뜁니다.")
        return
    else:
        # ID가 유효한 경우 토크 해제 시도
        dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, ID, ADDR_TORQUE_ENABLE, T_ENABLE)
        ...
        # 결과 확인 및 출력
        if dxl_comm_result != COMM_SUCCESS:
            print("[토크 해제 함수] %d번 모터, %s" % (ID, packetHandler.getTxRxResult(dxl_comm_result)))
        elif dxl_error != 0:
            print("[토크 해제 함수] %d번 모터, %s" % (ID, packetHandler.getRxPacketError(dxl_error)))
...
# 방향 설정
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, DXL1_ID, ADDR_DRIVE_MODE, DXL_NORMAL_MODE_VALUE)
...
while 1:
    ...
    # 1번 모터 제어
        ...
        print(f"모터{key_input}에 들어왔습니다. 종료하려면 0을 누르세요. 움직이려면 .와 /으로 움직이세요")
        while 1:
            key_input_1 = getch()
            if key_input_1 == '0':
                break
