...
set_init_position(DXL7_ID, 2500)
...
while 1:
    ...
    # 2번, 4번 모터 동시 제어 (로봇팔 세우기)
        ...
        print("----엘리베이터를 호출합니다. 초기 상태로 이동시키려면 ESC를 누르세요.")
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL2_ID, ADDR_GOAL_POSITION, 850)
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL4_ID, ADDR_GOAL_POSITION, 330)
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL7_ID, ADDR_GOAL_POSITION, 2523)
    else :
